...
class DataTransformation:
    def __init__(self, config_file_path: str = CONFIG_FILE_PATH):
        ...
        self.object_index = (
            COCO_NAMES[self.object_category]
        ...
        self.groundingdino_model = load_model(
            f"{GROUNDING_DINO_PATH}/groundingdino/config/GroundingDINO_SwinT_OGC.py",
            f"{GROUNDING_DINO_PATH}/weights/groundingdino_swint_ogc.pth",
        )
        self.groundingdino_model = self.groundingdino_model.to(self.device)

        self.SAM_ = SAM("sam2.1_b.pt")

    def get_bbox_w_yolo(self, img_path: str):
        res = self.YOLO_.predict(
            img_path, classes=[self.object_index - 1], verbose=False
        )[0]
    ...
    def transform_data(self) -> None:
        logger.info(
            f"{bin_colors.INFO}Further processing colmap output for {self.config.unique_id}{bin_colors.ENDC}"
        )
        try:
                ...
                for image_path in os.listdir(folder):
                    if image_path.endswith(".png"):
                        ...
                        if self.object_index:
                            ...
                            if bbox is not None:
                                logger.info(
                                    f"{bin_colors.OKCYAN}YOLO successfully detected {self.object_category} in {image_path} {bin_colors.ENDC}"
                                )
                                mask = self.get_mask_w_sam(bbox, _image_path)
                                self.apply_mask_n_save(_image_path, mask)
                                logger.info(
                                    f"{bin_colors.OKCYAN}SAM successfully segmented {self.object_category} in {image_path} {bin_colors.ENDC}"
                                )
                            else:
                                logger.info(
                                    f"{bin_colors.INFO}YOLO failed to detect, using GroundingDINO instead {bin_colors.ENDC}"
                                )
                                bbox = self.get_bbox_w_groundingdino(_image_path)
                                if bbox is not None:
                                    logger.info(
                                        f"{bin_colors.OKCYAN}GroundingDINO successfully detected {self.object_category} in {image_path} {bin_colors.ENDC}"
                                    )
                                    mask = self.get_mask_w_sam(bbox, _image_path)
                                    self.apply_mask_n_save(_image_path, mask)
                                    logger.info(
                                    ...
                                    logger.info(
                                        f"{bin_colors.INFO}YOLO and GroundingDINO failed to detect, applying OpenCV thresholding instead {bin_colors.ENDC}"
                                    )
                            ...
                            if bbox is not None:
                                logger.info(
                                    f"{bin_colors.OKCYAN}GroundingDINO successfully detected {self.object_desc} in {image_path} {bin_colors.ENDC}"
                                )
                                mask = self.get_mask_w_sam(bbox, _image_path)
                                self.apply_mask_n_save(_image_path, mask)
                                logger.info(
                                    f"{bin_colors.OKCYAN}SAM successfully segmented {self.object_desc} in {image_path} {bin_colors.ENDC}"
                                )
                            else:
                                # TODO - Change this to use Trellis
                                logger.info(
                                    f"{bin_colors.WARNING}Using CV2 to detect {self.object_desc.capitalize()} in {image_path}, may not be accurate{bin_colors.ENDC}"
                                )
                                self.process_with_cv2(_image_path)
        except Exception as e:
            logger.error(
                f"{bin_colors.ERROR}Error processing colmap output for {self.config.unique_id}{bin_colors.ENDC}"
            )
            raise e
        logger.info(
            f"{bin_colors.SUCCESS}Further processing of {self.config.unique_id} completed!{bin_colors.ENDC}"
        )
