...
try:
    logger.info(
        f"{bin_colors.OKCYAN}Uploading {config.unique_id} result to database{bin_colors.ENDC}"
    )

    with open(
        os.path.join(config.output_dir, config.unique_id, "splat.ply"), "rb"
    ) as f:
        supabase.storage.from_("korokoro_bucket").upload(
            file=f,
            path=f"{config.unique_id}/splat.ply",
...
try:
    logger.info(
        f"{bin_colors.OKCYAN}Updating product status: {config.unique_id}{bin_colors.ENDC}"
    )
    supabase.table("products").update({"status": "DONE"}).eq(
        "unique_id", config.unique_id
    ).execute()
except Exception as e:
    logger.error(
        f"{bin_colors.ERROR}Failed to update product status: {e}{bin_colors.ENDC}"
    )
    raise e
